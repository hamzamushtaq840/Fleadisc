...
const PublicInfo = () => {
    ...
    const userInfoQuery = useQuery(['userDataPublic', id], () => axios.get(`/user/${id}`), {
        onSuccess: (res) => {
        },
        onError: (error) => {
            console.log(error);
    ...
    if (userInfoQuery.isLoading && !userInfoQuery.data) {
        ...
        return (
            <>
                <div className='flex mx-[20px] xsm:justify-between sm:justify-between px-[10px] mt-[25px] gap-[0.938em] justify-center md:gap-[5.5em] lg:gap-[8.4em] xl:gap-[8em] 2xl:gap-[10em]'>
